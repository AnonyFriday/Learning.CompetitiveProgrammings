...
int defenseKindom(int w, int h, vector<Tower>& positions) {
  if (positions.empty()) {
    return w * h;
  }
  ...
  for (size_t i = 1; i < xs.size(); i++) {
    maxGapX = max(maxGapX, xs[i] - xs[i - 1] - 1);
  }

  for (size_t i = 1; i < ys.size(); i++) {
    maxGapY = max(maxGapY, ys[i] - ys[i - 1] - 1);
  }

  return maxGapX * maxGapY;
}
...
int main() {
  ...
  int result = defenseKindom(15, 8, arr);
