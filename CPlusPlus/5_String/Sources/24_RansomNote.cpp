...
/*
        Using array an mapping capture
*/
bool checkRansomNote(
    const string& ransomNote,
    const string& magazine) {
  if (magazine.empty() || ransomNote.empty()) return false;

  // Using unordered_map to count posibility
  int v[256] = {0};
...
/*
        Using hashset for it
*/
bool checkRansomNote1(
    const string& ransomNote,
    const string& magazine) {
  if (magazine.empty() || ransomNote.empty()) return false;
...
int main() {
  ...
  cout << checkRansomNote(ransomNote, magazine) << endl;
  cout << checkRansomNote1(ransomNote, magazine);
