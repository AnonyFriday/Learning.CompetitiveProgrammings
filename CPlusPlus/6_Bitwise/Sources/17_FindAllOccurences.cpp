...
/*
        time: n^2 (callstack + vector copy)
        space: n^2 (callstack + vector copy)
*/
vector<int> findAllOccurences(
  ...
  // Variables at each call stack
  // Get from the last, and push from the beginning
  size_t lastEl = v.back();
  size_t idx = v.size() - 1;
  v.pop_back();

  vector<int> results = findAllOccurences(k, v);

  // Conduct after returned from top of stack
  if (lastEl == k) {
...
int main() {
  ...
  auto result = findAllOccurences(k, inputs);
