...
long long int optimizedMod(long long int a, long long int b, const int& mod) {
  if (a == 0) return 0;
  if (b == 0) return 1;
  if (b == 1) return a % mod;

  long long int base = a % mod;
  long long int res = 1;
  while (b > 0) {
    if ((b & 1) > 0) {
...
/*
        After writing down on the paper, I come with the solution:
                result = (pow(result, 10) % M * (a^i) % M) %M

        Apply the fomular to reduce the overflow:
                (a*b) % M = (a%M * b%M) % M

*/
long long int superPow(const int& a, const vector<int>& b, const int& mod = 1337) {
  if (a == 0) return 0;
  if (b.size() == 1 && b[0] == 1) return a % mod;
  if (b.size() == 1 && b[0] == 0) return 1;

  size_t i = 0;  // index in b
  long long int result = 1;
  while (i < b.size()) {
    // result = ((pow(result, 10) % mod) * (pow(a, b[i]) % mod)) % mod;

    // Apply the multiplication in exponentiation on pow(result, 10)
    long long int powResult = optimizedMod(result, 10, mod);
    long long int powDigit = optimizedMod(a, b[i], mod);
    result = ((powResult % mod) * (powDigit % mod)) % mod;
    i++;
  }
...
int main() {
  ...
  outputVector(b);
  cout << "Test optimizedMod: " << optimizedMod(2, 0, 1337) << endl;
