...
Take the minimum number of steps to reach the earth at the level k, since number of jumps is a power of 2

e.g. steps = 7 = 4 + 2 + 1
--> for total number of steps, if we can seperate into the jumps, each jump is a power of 2
--> 7 = 0000 0111 --> since counting set bits will give the result

Using Brian Kernighan’s Algorithm to count bits
- eliminate right most bit per iteration using n & (n-1)
*/
int minimumJumpsInPowerOf2(long long int k) {
  if (k <= 0) return 0;
  if (k == 1) return 1;
...
int main() {
  ...
  int a;
  cin >> a;
  cout << "Min jumps: " << minimumJumpsInPowerOf2(a);
