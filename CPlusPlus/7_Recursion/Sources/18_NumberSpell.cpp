...
/*
        Time: length of num = log base 10 of n
        space: length of num = log base 10 of n
*/
string numberSpell(long long int a) {
  // special case
  if (a == 0) return "zero";

  // create a closure, and return later in the body of numberSpell
  function<string(long long)> solve = [&](long long num) -> string {
    if (num <= 0) {
      return "";
    }
    ...
    string subNum = solve(num / 10);

    // if it's a last digit
    if (subNum.empty()) {
      return digitStr;
    }

    return subNum + " " + digitStr;
  };
