...
bool validParentheses(const string& s) {
  ...
  for (const char& c : s) {
    // if found the counterpart, then check on top to match 1 pair
    if (parens.count(c)) {
      if (!temp.empty() && (temp.top() == parens[c])) {
        temp.pop();
        continue;
      }

      // e.g. stack: ({ <-- =, { not equals to =, hence we return false immediately
      return false;
...
int main() {
  ...
  string s;
  getline(cin, s);
  cout << "Is Valid: " << validParentheses(s);
