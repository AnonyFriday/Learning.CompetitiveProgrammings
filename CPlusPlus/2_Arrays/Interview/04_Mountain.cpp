...
// 1. define the peak
// 2. at that peak, count left and right till reaching the next peak
int highestMountain(vector<int>& arr) {
  ...
  for (int i = 1; i < n - 1; i++) {
    int count = 1;  // already count the peak
    int el = arr[i];

    // this is a peak
    if (arr[i - 1] < el && el > arr[i + 1]) {
      int j = i;

      // 8 1 2 3 5 4 2 1 9 10 1 1
      ...
      // count from right and also accelerate the i
