...
double slidingWindow(const vector<int>& v, int k) {
  if (v.empty()) return 0.0f;
  if (v.size() < k) return 0.0f;
...
/*
        Using prefix sum
*/
double prefixSum(const vector<int>& v, int k) {
  if (v.empty()) return 0.0f;
  if (v.size() < k) return 0.0f;

  double maxEl = -DBL_MAX;

  // padding 0 to avoid index overflow
  ...
  for (size_t i = 0; i + k < prefix.size(); i++) {
    double sum = prefix[i + k] - prefix[i];
    // cout << prefix[i + k] << "-" << prefix[i];
    maxEl = std::max(maxEl, sum / (double)k);
  }
...
int main() {
  ...
  double result = prefixSum(v, k);
  double result1 = slidingWindow(v, k);
