...
vector<bool> kidsWithCandies(vector<int>& candies, int extraCandies) {
  ...
  if (extraCandies < 1 || extraCandies > 50) return result;
  if (candies.empty()) return result;

  // find max
  int preMax = *std::max_element(candies.begin(), candies.end());

  for (size_t i = 0; i < candies.size(); i++) {
    result[i] = (candies[i] + extraCandies) >= preMax;
  }
...
int main() {
  ...
  // logic
  vector<bool> result = kidsWithCandies(arr, extras);
